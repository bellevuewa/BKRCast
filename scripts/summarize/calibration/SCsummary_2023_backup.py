...
# 05/01/2025
# Modified L237-L241 to save memory consume.

# 05/01/2025
# Modified the following to tailor to the 2023 survey features
...
def DistrictSummary(data1, data2, name1, name2, location, districtfile):
    ...
    writer = pd.ExcelWriter(location + '/DistrictReport_2023.xlsx', engine = 'xlsxwriter')
    ...
    colwidths = getmaxwidths(location + '/DistrictReport_2023.xlsx')
...
def WorkFAZSummary(data1, location):
...
def ModeChoice(data1, data2, data3, name1, name2, name3, location, districtfile, subarea=None):
    start = time.time()
    ...
    trip_ok_3 = data3['Trip'][['travtime', 'travdist', 'travcost', 'mode', 'hhno', 'pno', 'trexpfac', 'dpurp']].\
        query('travtime>0 and travtime<200')
    merge_per_hh_1 = pd.merge(data1['Person'][['hhno','psexpfac']], data1['Household'][['hhno', 'hhtaz']], 'outer', on = 'hhno')
    merge_per_hh_2 = pd.merge(data2['Person'][['hhno','psexpfac']], data2['Household'][['hhno', 'hhtaz']], 'outer', on = 'hhno')
    merge_per_hh_3 = pd.merge(data3['Person'][['hhno','psexpfac']], data3['Household'][['hhno', 'hhtaz']], 'outer', on = 'hhno')    
    
    if subarea == "BKR":
        taz_subarea = districtfile.query('Region == "BKR"')['TAZ'].unique()
        merge_per_hh_1 = merge_per_hh_1.loc[merge_per_hh_1['hhtaz'].isin(taz_subarea)]
        merge_per_hh_2 = merge_per_hh_2.loc[merge_per_hh_2['hhtaz'].isin(taz_subarea)]
        merge_per_hh_3 = merge_per_hh_3.loc[merge_per_hh_3['hhtaz'].isin(taz_subarea)]

        hhs_1_subarea = data1['Household'][['hhno', 'hhtaz']].loc[data1['Household']['hhtaz'].isin(taz_subarea), 'hhno']
        hhs_2_subarea = data2['Household'][['hhno', 'hhtaz']].loc[data2['Household']['hhtaz'].isin(taz_subarea), 'hhno']
        hhs_3_subarea = data3['Household'][['hhno', 'hhtaz']].loc[data3['Household']['hhtaz'].isin(taz_subarea), 'hhno']

        tour_ok_1 = tour_ok_1.loc[tour_ok_1['hhno'].isin(hhs_1_subarea)].copy()
        tour_ok_2 = tour_ok_2.loc[tour_ok_2['hhno'].isin(hhs_2_subarea)].copy()
        trip_ok_1 = trip_ok_1.loc[trip_ok_1['hhno'].isin(hhs_1_subarea)].copy()
        trip_ok_2 = trip_ok_2.loc[trip_ok_2['hhno'].isin(hhs_2_subarea)].copy()
        trip_ok_3 = trip_ok_3.loc[trip_ok_3['hhno'].isin(hhs_3_subarea)].copy()

        fname = 'ModeChoiceReport_BKR_2023.xlsx'
    else:
        fname = 'ModeChoiceReport_2023.xlsx'
    ...
    #Write DataFrames to Excel Fil
    with pd.ExcelWriter(os.path.join(location, fname), engine = 'xlsxwriter') as writer:
    ...
    colwidths=getmaxwidths(os.path.join(location, fname))
...
def report_compile(h5_results_file, h5_results_name,
    ...
    if run_mode_choice_report == True:
        ModeChoice(data1, data2, data3, h5_results_name, h5_comparison_name, h5_fullsurvey_name, report_output_location, zone_district)
        ModeChoice(data1, data2, data3, h5_results_name, h5_comparison_name, h5_fullsurvey_name, report_output_location, zone_district, "BKR")
