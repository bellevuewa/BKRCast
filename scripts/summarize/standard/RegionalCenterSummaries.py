...
def Thousands_Comma_Insertifier_9000(number):
    ...
    for i in range(num_commas):
        integer_list.append(integer_part[3*i + digits_before_comma + 1: 3*i + digits_before_comma + 4])
...
miles_traveled = pd.DataFrame(index = center_list, columns = ['VMT', 'PMT', 'VMT per Person', 'PMT per Person'])
...
for time_range in times:
    time_df = pd.DataFrame(index = center_list, columns = ['Arrivals', 'Departures', 'Change'])
...
for center in center_list:
    ...
    if center is not 'Total':
        trip_hh_center = pd.merge(data['Trip'][['dpurp', 'mode', 'hhno', 'trexpfac', 'travdist', 'dorp', 'pno']],
                                  hh_center, on = ['hhno', 'pno']).query('NAME == "' + str(center) + '"')
    else:
        trip_hh_center = pd.merge(data['Trip'][['dpurp', 'mode', 'hhno', 'trexpfac', 'travdist', 'dorp', 'pno']],
                                  hh_center, on = ['hhno', 'pno'])
    total_trips_center = trip_hh_center['trexpfac'].sum()
    mode_share = (trip_hh_center[['mode', 'trexpfac']].groupby('mode').sum()['trexpfac'] / total_trips_center).round(3)
    ...
    work_trips_center = trip_hh_center[['dpurp', 'trexpfac', 'mode']].query ('dpurp == "Work"')
    total_work_trips_center = work_trips_center['trexpfac'].sum()
    work_trips_mode_share = (work_trips_center[['mode', 'trexpfac']].groupby('mode').sum()['trexpfac'] / total_work_trips_center).round(3)
    ...
    if center is not 'Total':
        trip_work_center = pd.merge(data['Trip'][['dpurp', 'mode', 'hhno', 'trexpfac']],
                                    work_center, on = 'hhno').query('NAME == "' + str(center) + '" and dpurp == "Work"')
    else:
        trip_work_center = pd.merge(data['Trip'][['dpurp', 'mode', 'hhno', 'trexpfac']],
                                    work_center, on = 'hhno')
    total_trips_center = trip_work_center['trexpfac'].sum()
    work_dest_mode_share = (trip_work_center[['mode', 'trexpfac']].groupby('mode').sum()['trexpfac'] / total_trips_center).round(3)
    regional_summaries.update({'Work Destination Mode Split': work_dest_mode_share})

    for mode in work_dest_mode_share.index:
        work_dest_mode_split[mode][center] = regional_summaries['Work Destination Mode Split'][mode]

    driver_trips_center = trip_hh_center[['dorp', 'travdist', 'trexpfac']].query('dorp == "Driver"')
    vmt = (driver_trips_center['travdist'].multiply(driver_trips_center['trexpfac'])).sum()
    try:
    ...
    regional_summaries.update({'VMT': vmt, 'PMT': pmt, 'VMT per Person': vmt_per_person, 'PMT per Person': pmt_per_person})
    ...
    arrivals_by_time = arrivals[['arr_hr', 'trexpfac']].groupby('arr_hr').sum()['trexpfac'].round(0)

    total_arrivals = arrivals['trexpfac'].sum()
    try:
        a_mode_split = arrivals[['mode', 'trexpfac']].groupby('mode').sum()['trexpfac'] / total_arrivals
    except ZeroDivisionError:
        pass
    for mode in a_mode_split.index:
        arrival_mode_split[mode][center] = a_mode_split[mode].round(3)

    departures = data['Trip'][['opcl', 'dpcl', 'deptm', 'mode', 'trexpfac']].query('opcl in @pcl_list and dpcl not in @pcl_list')
    departures['dep_hr'] = min_to_hour(departures['deptm'], 3)
    departures_by_time = departures[['dep_hr', 'trexpfac']].groupby('dep_hr').sum()['trexpfac'].round(0)

    total_departures = departures['trexpfac'].sum()
    try:
        d_mode_split = departures[['mode', 'trexpfac']].groupby('mode').sum()['trexpfac'] / total_departures
    except ZeroDivisionError:
        pass
    for mode in a_mode_split.index:
    ...
    for time_range in times:
        ...
        trips_by_time[time_range]['Change'][center] = arrivals_by_time[time_range] - departures_by_time[time_range]
...
for center in center_list:
    try:
        miles_traveled['VMT/PMT Ratio'][center] = miles_traveled['VMT'][center] / miles_traveled['PMT'][center]
    except ZeroDivisionError:
        continue
...
for column in miles_traveled.columns:
    if column == 'VMT' or column == 'PMT':
        miles_traveled[column] = miles_traveled[column].round(0).apply(int).apply(Thousands_Comma_Insertifier_9000)
    else:
        miles_traveled[column] = miles_traveled[column].round(3)

def add_pie_charts(writer, sheet_name):
    colordict = {'SOV': '#C00000', 'HOV2': '#C06000', 'HOV3+': '#C00060', 'Walk': '#008000', 'Bike': '#0040C0', 'Transit': '#757575', 'School Bus': '#FFD800'}
    ...
    for i in range(worksheet.dim_rowmax):
        ...
        chart.add_series({'name': [sheet_name, i + 1, 0],
...
for i in range(len(times)):
    if i == 0:
        trips_by_time[times[i]].to_excel(excel_writer = writer, sheet_name = 'Arrivals and Departures by Hour',
                                         na_rep = '-', startrow = 1)
    else:
        trips_by_time[times[i]].to_excel(excel_writer = writer, sheet_name = 'Arrivals and Departures by Hour',
                                         na_rep = '-', startrow = 1, startcol = 4*i + 1, index = False)
...
for sheet in writer.sheets:
    ...
    for colnum in range(worksheet.dim_colmax + 1):
        ...
        worksheet.conditional_format(1, colnum, 27, colnum, {'type': 'top', 'value': '5', 'format': top_5_format})
        worksheet.conditional_format(1, colnum, 27, colnum, {'type': 'bottom', 'value': '5', 'format': bot_5_format})
