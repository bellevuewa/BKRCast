...
def line_to_line_transfers(emme_project, tod) -> pd.DataFrame:
    ...
    for line in network.transit_lines():
        transit_line_list.append({'line':line.id, 'mode':line.mode.id, 'description': line.description, 'route_name':line.description.split('-')[0]})
    transit_lines = pd.DataFrame(transit_line_list)
    transit_lines['lindex'] = transit_lines.index + 1
    transit_lines=transit_lines[['lindex', 'line', 'mode', 'description', 'route_name']]
    ...
    for class_name in ['trnst','commuter_rail','ferry','litrat','passenger_ferry']:
        ...
        traversal_df = traversal_df.rename(columns={'line':'from_line_id', 'mode':'from_mode', 'description':'from_description', 'route_name':'from_route_name'})
        ...
        traversal_df = traversal_df.rename(columns={'line':'to_line_id', 'mode':'to_mode', 'description':'to_description', 'route_name':'to_route_name'})
        traversal_df.drop(columns=['lindex'], inplace = True)
        df_list.append(traversal_df)
        os.remove('outputs/transit/traversal_results.txt')
        
    df = pd.concat(df_list)
    df = df.groupby(['from_line', 'to_line']).agg({'from_line_id' : 'first', 'from_description' : 'first', 'from_route_name': 'first', 'to_line_id' : 'first', 'to_description' : 'first', 'to_route_name' : 'first',  'from_mode' : 'first', 'to_mode' : 'first', 'boardings' : 'sum'})
    df.reset_index(inplace = True)
    df.drop(columns = ['from_line', 'to_line'], inplace = True)
    df['tod'] = tod
    return df
...
def main():
    ...
    for _path in [input_config.transit_line_path, input_config.transit_node_path, input_config.transit_segment_path, input_config.network_results_path, input_config.transit_demand_path]:
    ...
    zero_demand_list = []
    with pd.ExcelWriter(os.path.join(input_config.project_folder, input_config.transit_demand_path), engine = 'xlsxwriter') as writer:
        readmesheet = writer.book.add_worksheet('readme')
        readmesheet.write(0, 0, str(datetime.datetime.now()))
        readmesheet.write(1, 0, 'model folder')
        readmesheet.write(1, 1, input_config.project_folder)
        readmesheet.write(4, 0, 'notes')
        readmesheet.write(5, 0, '1. This file is generated by network_summary.py.')
        readmesheet.write(6, 0, '2. This file contains transit line OD tables for select lines in BKR area. Each sheet is for one line and one time period. The OD table is in sparse format with three columns: i_node, j_node, and demand. i_node and j_node are the nodes at the end of each transit segment. demand is the number of trips on that segment. Note that the same transit line may have different OD tables in different time periods due to different transit assignment results.')               
                ...
                if tod_hour in transit_line_od_period_list: 
                    for route_name in emme_config.transit_line_for_od_list:
                        ...
                        for line_id, name in line_ids.items():
                            readmesheet.write(readmesheet_row, 0, f'{line_id} ({name}) in time period {tod_hour}')
                            readmesheet_row += 1
                            ...
                            for class_name in ['trnst','commuter_rail','ferry','litrat','passenger_ferry']:
                                ...
                                filepath = 'outputs/transit/line_od/' + str(line_id) + '_'+ class_name + "_" + tod_hour + '.csv'
                                my_project.export_matrix('mfeline', filepath)
                                matrix, df = data_wrangling.od_list_to_matrix_numpy(filepath)
                                wksheetname = str(line_id) + '_'+ class_name + "_" + tod_hour
                                if df is not None:
                                    df.to_excel(writer, sheet_name = wksheetname, index = True)
                                else:
    ...
    with pd.ExcelWriter(input_config.transit_line_path, engine = 'xlsxwriter') as writer:
        wksheet = writer.book.add_worksheet('readme')
        wksheet.write(0, 0, str(datetime.datetime.now()))
        wksheet.write(1, 0, 'model folder')
        wksheet.write(1, 1, input_config.project_folder)
        wksheet.write(2, 0, 'notes')
        wksheet.write(3, 0, f'This file is generated by network_summary.py. It contains transit summary results for all transit lines in the region for all time periods with transit assignment results. If the file is for transit segments, it also includes boarding/alighting numbers on each segment.')
        df_transit_line.to_excel(writer, sheet_name = 'transit_line', index = False)

        for tod, table in df_transit_line.groupby('tod'):
            table.to_excel(writer, sheet_name = f'{tod}', index = False)

        df_transit_line['boarding_time'] = df_transit_line['time'] * df_transit_line['boardings']
        daily = df_transit_line[['route_name', 'mode', 'boarding_time', 'boardings', 'time']].groupby('route_name').agg({'mode':'first', 'boardings': 'sum', 'boarding_time': 'sum', 'time':'mean'}).reset_index()
        daily['weighted_time'] = daily['boarding_time'] / daily['boardings']
        daily.loc[daily['boardings'] == 0, 'weighted_time'] = daily['time']
        daily.drop(columns = ['boarding_time', 'time'], inplace = True)
        daily.to_excel(writer, sheet_name = 'daily_summary', index = False)

    with pd.ExcelWriter(input_config.transit_node_path, engine = 'xlsxwriter') as writer:
        wksheet = writer.book.add_worksheet('readme')
        wksheet.write(0, 0, str(datetime.datetime.now()))
        wksheet.write(1, 0, 'model folder')
        wksheet.write(1, 1, input_config.project_folder)
        wksheet.write(2, 0, 'notes')
        wksheet.write(3, 0, f'This file is generated by network_summary.py. It contains transit summary results for all transit stops in the region for all time periods with transit assignment results. If the file is for transit segments, it also includes boarding/alighting numbers on each segment.')   
        
        df_transit_node.to_excel(writer, sheet_name = 'transit_node', index = False)

        for tod, table in df_transit_node.groupby('tod'):
            table.to_excel(writer, sheet_name = f'{tod}', index = False)
        ...
        # aggregate boarding/alightings by TOD and daily for transit hubs, and export to separate sheets
        hub_wksheet = writer.book.add_worksheet('hub_summary') 
        readme_row = wksheet.dim_rowmax + 2
        wksheet.write(readme_row, 0, 'Transit Hub Definition')
        readme_row += 1
        row = 0       
        for hub_name, node_list in transit_hub_lookup_dict.items():
            hub_df = df_transit_node.loc[df_transit_node['node_id'].isin(node_list)].copy()
            if hub_df.empty:
                continue
            
            hub_wksheet.write(row, 0, f'{hub_name} summary')
            # calculate daily summary for the hub and append to the end of hub_df
            hub_df = hub_df.groupby('tod').agg({'node_subarea': 'first'} | {col: 'sum' for col in cols}).reset_index()
            numeric_cols = hub_df.select_dtypes(include='number').columns
            summary_row = hub_df[numeric_cols].sum()
            summary_row['tod'] = 'daily'
            summary_row['node_subarea'] = ''
            hub_df = pd.concat([hub_df, pd.DataFrame(summary_row).T], ignore_index = True)

            row += 1
            hub_df.to_excel(writer, sheet_name = 'hub_summary', index = False, startrow = row)
            row += hub_df.shape[0] + 3

            wksheet.write(readme_row, 0, f'{hub_name} : {node_list}', )
            readme_row += 1
    ...
    with pd.ExcelWriter(input_config.transit_transfer_file, engine = 'xlsxwriter') as writer:
        wksheet = writer.book.add_worksheet('readme')
        wksheet.write(0, 0, str(datetime.datetime.now()))
        wksheet.write(1, 0, 'model folder')
        wksheet.write(1, 1, input_config.project_folder)
        wksheet.write(2, 0, 'notes')
        wksheet.write(3, 0, f'This file is generated by network_summary.py. It contains transit transfer summary results for all transit lines in the region for all time periods with transit assignment results. Each row represents a transfer from one line to another line. The columns include the origin line id and name, destination line id and name, number of transfers between the two lines, and the time period of the transfer.')   
        
        df_transit_transfers.to_excel(writer, sheet_name = 'transit_transfers', index = False)  
        for tod, table in df_transit_transfers.groupby('tod'):
            table.to_excel(writer, sheet_name = f'{tod}', index = False) 

        daily = df_transit_transfers.groupby(['from_line_id', 'to_line_id']).agg({'from_mode': 'first', 'to_mode': 'first', 'boardings': 'sum'}).reset_index()     
        daily.to_excel(writer, sheet_name = 'daily_summ_by_lineid', index = False)

        daily = df_transit_transfers.groupby(['from_route_name', 'to_route_name']).agg({'from_mode': 'first', 'to_mode': 'first', 'boardings': 'sum'}).reset_index()
        daily.to_excel(writer, sheet_name = 'daily_summ_by_route', index = False)
