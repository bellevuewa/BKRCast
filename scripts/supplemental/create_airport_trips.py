...
def load_skim_data():
    ...
    for skim in skim_list:
        for submode in submode_list:
            ...
            am_skim = load_skims(emme_config.am_skim_file_loc, table=skim_name, 
                             divide_by_100=True) 
            pm_skim = load_skims(emme_config.pm_skim_file_loc, table=skim_name, 
                             divide_by_100=True)
            skim_dict['transit'][skim_name] = (am_skim + pm_skim) *.5
    for skim in fare_list:
        skim_dict['transit'][skim] = load_skims(emme_config.am_skim_file_loc, table=skim, divide_by_100=True)
...
-       - Airport Trip Ends WBO (production zone) = 0.01486 * employment
    ...
    taz_df.rename(columns={'hhtaz': 'BKRCastTAZ'}, inplace=True)
    ...
    parcel_df.rename(columns={'TAZ_P': 'BKRCastTAZ'}, inplace=True)
    
    trips = parcel_df.merge(taz_df, on='BKRCastTAZ')
...
def create_demand_matrix(airport_trips, dict_zone_lookup):
    ...
    for rec in airport_trips.iterrows():
        ...
        origin = dict_zone_lookup[rec[1].BKRCastTAZ]
