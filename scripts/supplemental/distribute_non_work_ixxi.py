...
def calc_fric_fac(cost_skim, dist_skim, _coeff_df):
    ...
    for index, row in _coeff_df.iterrows():
        ...
        friction_fac_dic[row['purpose']][emme_config.MIN_EXTERNAL:] = 0
        friction_fac_dic[row['purpose']][:,[x for x in range(emme_config.MIN_EXTERNAL, len(cost_skim))]] = 0
...
def main():
    ...
    am_dist_skim = load_skims(emme_config.am_skim_file_loc, mode_name='svtl1d', divide_by_100=True)
    pm_cost_skim = load_skims(emme_config.pm_skim_file_loc, mode_name='svtl2g')
    pm_dist_skim = load_skims(emme_config.pm_skim_file_loc, mode_name='svtl1d', divide_by_100=True)
