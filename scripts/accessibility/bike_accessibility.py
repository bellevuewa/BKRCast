...
def main():  
    ...
    parcels = data_wrangling.load_parcel_data_without_JBLM_jobs(os.path.loin(input_config.parcels_file_folder, access_config.parcels_file_name))
    ...
    parK_access_by_TAZ = park_accessibility_df[park_accessibility_df.select_dtypes(include = ['number']).columns].groupby('BKRCastTAZ').sum() 
