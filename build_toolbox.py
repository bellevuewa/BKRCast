...
def build_toolbox(fp, source, title='BKRCast Toolbox', namespace='bkrcast', consolidate=False):
    print("------------------------")
...
if __name__ == "__main__":
    ...
    parser.add_argument('-t', '--title', help="Title of the Toolbox. Default is 'bkrcast Toolbox'")
    parser.add_argument('-n', '--namespace', help="The initial namespace. Default is 'bkrcast'")
    ...
    toolbox_title = "BKRCast Toolbox" if args.title is None else args.title
    toolbox_fp = os.path.join(current_folder, 'BKRCast_Toolbox.mtbx') if args.path is None else args.path
    toolbox_namespace = 'bkrcast' if args.namespace is None else args.namespace
    consolidate_flag = args.consolidate

    build_toolbox(toolbox_fp, src_folder, toolbox_title, toolbox_namespace, consolidate_flag)

    # copy newly created mtbx file to each tod project folder.
    for tod in emme_config.tods:
        dest_path = os.path.join(input_config.project_folder, 'projects', tod)
