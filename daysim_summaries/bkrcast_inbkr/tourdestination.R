...
merge_skims <- function(tourdata)
{
  ...
  for(k in 1:4)
  {
    skims <- read.table(skimsfiles[k],header=F)
    setnames(skims,c("totaz","tdtaz","H1TIM","H1DIS","H1COST"))
...
if(prepSurvey)
{
  ...
  {
    purp <- i+2 
    write_tables(tourdestmodelout[i],survtourdata[pdpurp2 == purp],tourdestmodelfile,"survey")
  }
  purp <- 8
...
if(prepDaySim)
{
  ...
  {
    purp <- i+2 
    write_tables(tourdestmodelout[i],dstourdata[pdpurp2 == purp],tourdestmodelfile,"daysim")
  }
  purp <- 8
