...
#data folder
wd = os.path.join(project_folder, "daysim_summaries")
...
for index in range(0,len(infile_names)):
    ...
    for run_name in run_names:
        ...
        infile = os.path.join(wd, "output", f"{infile_names[index].split('.')[0]}_{run_name.split('_')[1]}.xlsm")
        outfile = os.path.join(wd, "compare", f"{infile_names[index].split('.')[0]}_{run_name.split('_')[1]}.xlsm")
        ...
        inworkbook = openpyxl.load_workbook(infile, keep_vba=True)
        ...
        if (infile_names[index]=="TripDestination.xlsm"):
            ...
            for purp in purpose:
                ...
                if run_name == 'soundcast':
                    ...
                    value = insheet.cell(row=11, column=11).value  # openpyxl uses 1-based indexing
                    ...
                    if (purp=='Home'):
                        ...
                        value = insheet.cell(row=19, column=22).value  # openpyxl uses 1-based indexing
                ...
                value = insheet.cell(row=11, column=12).value  # openpyxl uses 1-based indexing
                ...
                if (purp=='Home' and run_name != 'soundcast'):
                    ...
                    value = insheet.cell(row=19, column=26).value  # openpyxl uses 1-based indexing
                ...
                for nrow in range(0,nrows):
                    ...
                    if (nrow!=9):  #header                
                        #get data 
                        data = [insheet.cell(row=nrow+1, column=7).value]  # openpyxl uses 1-based indexing
                        # data = [insheet.cell_value(nrow,6)]
                        data_survey = [insheet.cell(row=nrow+1, column=3).value]
            ...
            insheet = inworkbook[sheet_names[index]]
            nrows = insheet.max_row
